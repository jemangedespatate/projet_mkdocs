...
## Objectif
Dans ce deuxième TP, nous allons continuer notre découverte des Interfaces Homme-Machine (IHM) avec `tkinter`. Contrairement au TP précédent (Flappy Bird) où nous avons utilisé des formes géométriques dessinées par le code, nous allons cette fois-ci charger et utiliser de véritables images pour reconstruire étape par étape un jeu de *Space Invader*. 
